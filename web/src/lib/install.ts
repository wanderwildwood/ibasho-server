...
// its own in a Chromium browser where there is one, and in the usual browser where not.
export function desktopEntry(url: string): string {
  const exec =
    "sh -c 'for b in brave-browser brave chromium chromium-browser google-chrome " +
    'google-chrome-stable microsoft-edge vivaldi; do command -v $b >/dev/null 2>&1 && ' +
    `exec $b --app="${url}"; done; exec xdg-open "${url}"'`;
  return [
    '[Desktop Entry]',
    'Type=Application',
    'Name=Whereabouts',
